...
### なぜ6エージェントに分けるのか — コンテキスト汚染の防止

1エージェントで全フェーズを担うと、巨大なコードベースを解析する過程で
コンテキストウィンドウが溢れてアウトプットの品質が落ちます。
各エージェントは「自分の担当範囲のファイルだけ読む」ため、無関係な情報を抱え込みません。

```
❌ 単一エージェント:  ソース全体 + クラス + シーケンス + 例外 + 組み立て → コンテキスト爆発
✅ 6エージェント構成: 各フェーズが必要なファイルのみ読む → コンテキスト一定
```

### なぜ Architecture Analyst を最初に実行するのか — 共有コンテキストの先行確立

後続の3アナリスト（静的・動的・例外）はすべて `context.md` を読んでから処理を開始します。
`context.md` には「このプロジェクトはC# + Clean Architectureである」という共有知識が含まれており、
これがないと各アナリストが毎回プロジェクトファイルを走査する二重コストが発生します。

```
Architecture Analyst（先行・直列）
    ↓  context.md を生成
Static Analyst ─┐
Dynamic Analyst ─┤ 各自が context.md を読んで走査範囲を絞る
Exception Analyst ─┘
```

### なぜこの4軸（アーキテクチャ・静的・動的・例外）か — ソフトウェア設計書の標準構成

各フェーズは「読むファイルの種類」が明確に異なります。担当範囲を分けることで、
エージェントごとの走査対象を絞り込めます。

| フェーズ | 主に読むもの | 生成成果物 |
|---|---|---|
| **アーキテクチャ** | プロジェクトファイル・ディレクトリ構造・README | レイヤー構造図・依存関係 |
| **静的設計** | クラス/インターフェース定義・DBスキーマ・マイグレーション | クラス図・テーブル設計 |
| **動的設計** | コントローラー・サービス呼び出し・非同期処理 | シーケンス図・状態遷移図 |
| **例外設計** | try/catch ブロック・カスタム例外クラス・グローバルハンドラー | 例外階層・ハンドリング方針 |

### なぜ Assembler を独立させるのか — 責務の単一化

各アナリストは「分析に特化」し、ドキュメントの統合責務を持ちません。
Assembler は `.copilot-reverse/` 配下を**全部読む唯一のエージェント**で、
目次・相互参照・PlantUML のインライン展開など、全体を見ないと書けない要素を担当します。

出力フォーマット（AsciiDoc の構成順序・属性設定）を変えたい場合も、
Assembler だけを修正すれば済みます。

### なぜ中間ファイル（`.copilot-reverse/`）を経由するのか — オーケストレーターのコンテキスト保護

エージェント間でテキストを直接受け渡すと、オーケストレーターのコンテキストに
設計書の内容が展開されてしまいます。ファイル経由にすることで
**オーケストレーターはパスだけ管理**し、本文はサブエージェントが直接読みます。

```
❌ 直接受け渡し:  Analyst → "クラス図の内容全文..." → Orchestrator（コンテキスト肥大化）
✅ ファイル経由:  Analyst → .adoc ファイル書き込み → Orchestrator（"完了: path" の1行のみ受け取る）
```

また、中間ファイルが残ることで**特定フェーズのみの再実行**が可能になります。
クラス設計だけ更新したい場合は Static Analyst だけを再実行し、
その後 Assembler で再統合するだけで済みます。
