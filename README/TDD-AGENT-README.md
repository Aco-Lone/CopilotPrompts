...
### 1. オーケストレーションパターンを採用した理由

単一エージェントに全処理を任せると、会話コンテキストが肥大化して精度が劣化します。
本構成では処理を役割ごとに専門エージェントへ委譲し、**各エージェントのコンテキストを最小・独立に保つ**オーケストレーションパターンを採用しています。

```
単一エージェント方式（問題）          オーケストレーション方式（本構成）
─────────────────────────           ──────────────────────────────
設計書全文                            サブエージェントのコンテキストは
  + 全テストコード          →過大       各ロールの処理に必要な情報のみ
  + 全実装コード
  + レビュー結果 ...
```

### 2. ファイルハンドオフとソース参照トレースを採用した理由

エージェント間で情報を「コピーして渡す」と 2 つの問題が起きます。

| 問題 | 内容 |
|------|------|
| **コンテキスト膨張** | オーケストレーターのコンテキストに全ユニット情報が蓄積される |
| **情報欠落・劣化** | 情報をコピー・要約するたびに元の意図・詳細が失われる |

本構成では **設計書の該当セクションへの参照（ポインタ）だけをハンドオフ**し、
各サブエージェントが一次情報源（設計書原文）を直接読みます。
これにより情報欠落がなく、`source:` パスがトレーサビリティの証跡にもなります。

```
❌ コピー渡し                    ✅ ソース参照渡し
───────────────                  ──────────────────────────
"OrderService は〜します"  →     source: docs/design/order-service.md#place-order
（要約が丸まる）                   （サブエージェントが原文を直接読む）
```

### 3. context.md と units.md を分離した理由

設計書の情報には 2 種類あります。

| 種別 | 内容 | 更新頻度 |
|------|------|---------|
| **横断的制約** | 命名規則・DI方針・エラー処理方針 | 設計書全体で 1 回読めば十分 |
| **ユニット固有情報** | 各クラス・メソッドの仕様 | ユニットごとに必要な箇所のみ読む |

`context.md` に横断的制約を一度だけ抽出・集約し、全エージェントが参照します。
ユニット固有情報は `units.md` のポインタ経由で**必要な時に必要な箇所だけ**読むため、
設計書のサイズが増えてもサブエージェントのコンテキスト量が増加しません。

### 4. テストケースドキュメントを生成する理由

テストコード（`.cs`）だけでは「何を意図してテストしているか」がレビュアーに伝わりにくく、
C1（分岐網羅）の抜け漏れも機械的に確認しにくい問題があります。

`.copilot-tdd/test-cases/[N]-{ClassName}.md` として分岐一覧とカバレッジ充足状況を可視化することで、
コードレビュー前にテスト設計レベルでの確認を可能にしています。

### 5. 言語固有エージェントを差し替え可能にした理由

TDD のフロー制御（Red → Green → Refactor → Review → CI）は言語に依存しません。
言語固有なのは「テスト生成」と「実装生成」の 2 エージェントだけです。

オーケストレーターはワークスペースのプロジェクトファイルから言語を自動検出し、
`description` のキーワードマッチでサブエージェントを委譲するため、
**オーケストレーターを変更せず `{lang}-test-writer.agent.md` を追加するだけで他言語対応できます**。
